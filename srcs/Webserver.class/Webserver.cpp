...
#include "../IHTTPMessage.interface/Response.class/Response.hpp"
...
void Webserver::doWrite(int ind)
{
	...
	if (error_ < static_cast<int>(buf.size()))
	{
...
bool Webserver::requestIsFull(const std::string& request)
{
	...
	if (clKey != std::string::npos)
	{
		...
		size_t clValue;
		sscanf(request.substr(clStart, clEnd - clStart).c_str(), "%ld", &clValue);
